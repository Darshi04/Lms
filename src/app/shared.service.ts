  ...
  getCourses() {
    return this.http.get<{ msg: string, course: Course[] }>(`${this.baseUrl}/course`).pipe(
      tap((response: any) => {
        ...
        // Ensure 'course' is correctly mapped as 'courses' in the response
        if (response && Array.isArray(response.course)) {  // Check for 'course' array
          this.courses = response.course.map((course: any) => ({
            course_id: course.course_id,
            course_title: course.course_title,
            start_date: course.start_date,
            topic: course.topic.map((topic: any) => ({
              topic_model: topic.topics_topic,  // Mapping the topic name
              topic_completed: topic.topic_completed  // Mapping completion status
            }))
          }));
        } else {
